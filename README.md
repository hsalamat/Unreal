...
Level 07_04
1. If you want to fine-tune the look and feel of your scene, you can use what's called a post process volume and this gives you all sorts of post-processing effects. So what we need to do is add the volume to our scene and then we can work. Now a post process volume can work either on the entire scene or just on specific areas. So let's start with a specific area. 
2. We're going to do "quickly add to the project" into add Visual Effects, Post Process Volume. And it brings in this little cube here, which is basically just a volume. 
3. Now I can move, rotate, and scale that. So I'm going to move that over my house and scale it up so that the volume completely encloses the house.  
4. And then I'm going to go in on that patio so I can see this a little bit better. So maybe select this table, hit "F" to frame it, and then bring that camera out a little bit. So now my camera's inside that volume. 
5. And let's go ahead and select that PostProcessVolume and go into the details panel. Now in the details panel, you have a lot of different things that you can change but let's start off with exposure. So if I go under lens, you'll see we have an exposure. And if you can't ever find a value, you can always search for it in that search tab here. So if I type in exposure, you can see I'm getting all my exposure here. But let's just go ahead and find exposure. 
6. And what we want is we want to change the metering mode. So we can change it from auto-exposure to manual. And when I change it to manual, you can see how whole scene gets brighter. Well, I can make it darker or brighter. And so I'm changing the "exposure compensation". Now watch what happens if I move my camera back. Once I'm outside of that volume, my exposure goes back to what I had before, but as soon as I go into it, it changes. And so this volume can be used on an area-by-area basis to change how your scene looks. So if you go into, say, an inside versus outside look, you can have that take effect with a post process volume. 
7. Now I'm going to change my metering mode to auto-exposure basic. And then we can create what's called exposure compensation. So it'll automatically change the exposure. But then we can kind of knock it down a bit if we want
8. Now, in addition to exposure, we have a whole bunch of other stuff. We have what's called bloom. So if I want, I can turn on what's called bloom and give it kind of that effect. I'm going to go ahead and turn that off. 
9. We also have camera effects. I can change the shutter speed, the f-stop, the aperture. 
__DEFINITION__ In optics, the aperture of an optical system is a hole or an opening that primarily limits light propagated through the system. 
__DEFINITION__ Shutter speed is exactly what it sounds like – the speed at which the shutter of the camera closes. A fast shutter speed creates a shorter exposure — the amount of light the camera takes in —while a slow shutter speed gives the photographer a longer exposure.

__NOTE__ Aperture is one of the three sides of the Exposure Triangle, along with shutter speed and ISO. It is the interaction of all three that determines the exposure of an image.
...
__DEFINITION__ The “f” in f-stop stands for the focal length of the lens. While focal length itself refers to the field of view of a lens, f-stop is about how much light you allow to hit the sensor via the aperture opening.
10. I can change what's called local exposure. So I can change highlight or shadow contrast. I can add in lens flares. 

https://dev.epicgames.com/documentation/en-us/unreal-engine/post-process-effects-in-unreal-engine?application_version=5.4

11. I can do image effects. I can create a vignette. So if I want I can vignette this. And then as soon as I go outside of that, the vignette goes away. And once I'm in, there's a vignette. 
12. We also have depth of field effects. So we can turn on depth of field, give it a sensor size as well as a focal distance. And as soon as I bring that distance up to a number, let's say I put it at a 1, you'll see I'm getting a lot of depth of field. So I can maybe bring it up to number, say like maybe 30 or so. And you can see that as I get closer to objects, they come into focus and then that stuff in the background is out of focus. So let's maybe make this 50, knock it down a little bit. And so as you can see, we can get some very nice depth of field effects. And then we can turn that on or off. 
13. Color grading is really important. We can change the temperature of the scene. So we can make it hotter or cooler. We can do all sorts of color grading, everything from shadows and mid-tones and highlights to all sorts of film correction.  
14. We can change the values of global illumination. So as we go deeper into lumen, we can change all of that. 
15. So as you can see, this has a lot of different effects that we can do. 
16. Now I'm going to go back up to exposure and I'm just going to turn exposure compensation down. Just make the scene a little dark and then go out and you can see that it brightens up as soon as I get out of that volume. Now, if I wanted to, I could increase that volume. 
17. But if I want this to affect the entire scene, then I need to set another value. So what I'm going to do is keep this selected and then under search, I'm just going to type in the word "infinite". And what you can do is, in post process volume settings, you're going to have something called infinite extent. Click that on. And now whatever this post process volume has will be applied to the entire scene. So this is a way to get kind of a global effect. Just make sure you check that on. And if you check it off, you can use that volume in specific areas. So this is a very useful tool. I use it all the time. And I think it's a great way to just fine-tune and tweak the look of your renders.